...
>
    ...
    <div class="absolute top-6 right-6 bg-[#D4AF37]
                px-6 py-3 text-xl font-bold text-black shadow-lg">
    ...
    @if($availableCount > 0)
    <div class="absolute top-6 left-6 bg-green-500 text-white
                px-4 py-2 text-sm font-semibold rounded">
        {{ $availableCount }} Rooms Available
    </div>
    @else
    <div class="absolute top-6 left-6 bg-red-500 text-white
                px-4 py-2 text-sm font-semibold rounded">
        Sold Out
    </div>
    @endif
